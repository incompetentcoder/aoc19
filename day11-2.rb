...
a=File.open("inputday11-1").read.split(",").map{ |x| x.to_i}
a+=[0]*10000
panels=[0]*10000
length=100
painted=Hash.new
  ...
  outputs=[]
    ...
    while (ins=a[pos].to_s)!="99"
      ...
      when 4
        ...
        outputs.push(output)
        if outputs.length == 2
          panels[robopos[1]*length+robopos[0]] = outputs[0] == 1 ? 1 : 0
          painted[robopos]=1
          roboangle+= outputs[1] == 0 ? -Math::PI/2 : Math::PI/2
          robopos = [robopos[0]+(Math::sin(roboangle)).to_i,robopos[1]+(Math::cos(roboangle)).to_i]
          input = panels[robopos[1]*length+robopos[0]]
          outputs=[]
        end
        pos+=2
      when 5
